...
from .dynamic_visibility import (
    projection_visibility_confidence,
    roundtrip_reprojection_confidence,
    signed_occlusion_visibility_confidence,
)
...
def geometric_confidence(
        est_depth, ref_proj, src_proj, height, width, src_depth=None,
        mode='roundtrip', occ_abs_tol=2.0, occ_rel_tol=0.01,
        temperature=1.0):
    """Geometry confidence for dynamic visibility weighting."""
    if src_depth is None:
        return projection_visibility_confidence(
            est_depth, ref_proj, src_proj, height, width)
    if mode == 'roundtrip':
        return roundtrip_reprojection_confidence(
            est_depth, src_depth, ref_proj, src_proj, height, width)
    if mode == 'occlusion':
        return signed_occlusion_visibility_confidence(
            est_depth, src_depth, ref_proj, src_proj, height, width,
            occ_abs_tol=occ_abs_tol, occ_rel_tol=occ_rel_tol,
            temperature=temperature)
    raise ValueError("Unknown Geo confidence mode: {}".format(mode))
