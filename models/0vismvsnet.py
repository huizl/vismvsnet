...
class RegNet3D(nn.Module):
    """Lightweight 3D encoder-decoder for per-pair cost volume (8→16→8)."""
    ...
    def forward(self, x):
        ...
        x = F.relu(self.deconv1(conv2) + conv0, inplace=True)
        return x
...
class RegFuse(nn.Module):
    """Fused cost volume regularization + final score head."""
...
class VisMVSModel(nn.Module):
    ...
    def forward(self, imgs, proj_matrices, depth_values_orig):
        ...
        depth_1, conf_1, pairs_1 = self.stage1(
            ref_feat8, ref_proj, srcs_feat8, src_projs,
        ...
        depth_3, conf_3, pairs_3 = self.stage3(
            ref_feat2, ref_proj, srcs_feat2, src_projs,
