...
from models.vismvsnet_convnext import VisMVSModel
from utils import *
...
def _save_error_pfm_and_png(outdir, fname, error):
    ...
    error_col = cv2.applyColorMap((error_clip * 255).astype(np.uint8), cv2.COLORMAP_HOT)
    cv2.imwrite(png_path, error_col)



def _save_occlusion_pfm_and_png(outdir, fname, occ_logits):
    """Save occlusion map: sigmoid as PFM, grayscale PNG + JET colormap PNG.

    Grayscale: black = occluded, white = visible.
    JET: blue = visible, red = occluded.
    """
    occ_prob = 1.0 / (1.0 + np.exp(-occ_logits))  # sigmoid
    pfm_path = os.path.join(outdir, fname.format('.pfm'))
    gray_path = os.path.join(outdir, fname.format('.png'))
    jet_path = os.path.join(outdir, fname.format('_jet.png'))
    os.makedirs(os.path.dirname(pfm_path), exist_ok=True)
    save_pfm(pfm_path, occ_prob.astype(np.float32))
    # grayscale: black = occluded, white = visible
    visible = np.clip(1.0 - occ_prob, 0., 1.)
    cv2.imwrite(gray_path, (visible * 255).astype(np.uint8))
    # JET colormap: blue = visible (0), red = occluded (1)
    occ_clip = np.clip(occ_prob, 0., 1.)
    jet_col = cv2.applyColorMap((occ_clip * 255).astype(np.uint8), cv2.COLORMAP_JET)
    cv2.imwrite(jet_path, jet_col)
...
def save_depth():
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            # ---- extract stage 3 occlusion maps (per source view) ----
            occ_s3_list = []
            for src_idx, (_, _, occ_logits) in enumerate(outputs[2][1]):
                occ_up = F.interpolate(
                    occ_logits,
                    size=(H_orig, W_orig),
                    mode='bilinear', align_corners=False).squeeze(1)  # [B, H, W]
                occ_s3_list.append(occ_up.cpu().numpy())
            ...
            for b in range(B):
                ...
                # --- occlusion maps (stage 3, per source view) ---
                blank = fname.format('', '')
                scan_name, _, view_str = blank.rstrip('/').split('/')
                for src_idx, occ_np in enumerate(occ_s3_list):
                    occ_fname = scan_name + '/occ_stage3/' + view_str + '_src{}'.format(src_idx) + '{}'
                    _save_occlusion_pfm_and_png(args.outdir, occ_fname, occ_np[b])
...
def _save_error_maps(fname_blank, d1, d2, d3):
    """Try to load GT depth and save error maps for each stage.

    Returns dict with abs_sum, pixels, lt2, lt4, lt8 (or None if no GT).
    """
