...
from models.vismvsnet_conv_pid_geo import VisMVSModel, VisMVSLoss
from utils import *
import gc
import sys
import datetime
...
model = VisMVSModel(
    mode=args.vismode, use_geo=True, use_pid=True, geo_alpha=args.geo_alpha,
...
model.cuda()
model_loss = VisMVSLoss(occ_guide=False, use_pid=True, edge_weight=1.0)
...
def train_sample(sample, detailed_summary=False):
    ...
    outputs, final_depth, prob_maps, edge_maps = model(
        sample_cuda["imgs"], sample_cuda["proj_matrices"], depth_values)

    loss, scalar_outputs = model_loss(
        outputs, depth_gt, mask, depth_interval, edge_maps=edge_maps)
    loss.backward()
    optimizer.step()
