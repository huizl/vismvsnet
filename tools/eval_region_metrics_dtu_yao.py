...
MODEL_SPECS = {
    ...
    "geo_visibility": (
        "models.vismvsnet_geo",
        {"use_geo": True, "use_visibility_head": True},
    ),
    "geo_visibility_cascade": (
        "models.vismvsnet_geo",
        {
            "use_geo": True,
            "use_visibility_head": True,
            "use_visibility_propagation": True,
        },
    ),
...
def parse_args():
    ...
    parser.add_argument("--visibility_beta", type=float, default=0.5,
                        help="Residual learned-visibility gate strength in [0, 1].")
...
def build_model(args, model_type=None, geo_alpha=None):
    ...
    if model_type in ("geo", "geo_visibility", "geo_visibility_cascade"):
        kwargs.update(
            geo_confidence_mode=getattr(args, "geo_confidence_mode", "roundtrip"),
            geo_occ_abs_tol=getattr(args, "geo_occ_abs_tol", 2.0),
            geo_occ_rel_tol=getattr(args, "geo_occ_rel_tol", 0.01),
            geo_occ_temperature=getattr(args, "geo_occ_temperature", 1.0),
        )
    if model_type == "geo_visibility":
        kwargs.update(
            visibility_beta=getattr(args, "visibility_beta", 0.5),
            visibility_stages=(1,),
        )
    if model_type == "geo_visibility_cascade":
        kwargs.update(
            visibility_beta=getattr(args, "visibility_beta", 0.5),
            visibility_stages=(1,),
            visibility_stage_betas=(
                getattr(args, "visibility_beta", 0.5),
                getattr(args, "visibility_stage2_beta", 0.25),
                getattr(args, "visibility_stage3_beta", 0.1),
            ),
            visibility_residual_scales=(
                0.0,
                getattr(args, "visibility_stage2_residual_scale", 0.25),
                getattr(args, "visibility_stage3_residual_scale", 0.1),
            ),
        )
...
def save_csv(label, rows, filename, args):
    ...
    with open(filename, "w", newline="") as f:
        fieldnames = [
            ...
            "geo_occ_rel_tol", "geo_occ_temperature", "visibility_beta",
            "visibility_stage2_beta", "visibility_stage3_beta",
            "visibility_stage2_residual_scale", "visibility_stage3_residual_scale",
        ...
        for row in rows:
            writer.writerow(dict(
                ...
                if args.model_type in ("geo", "geo_visibility", "geo_visibility_cascade") else "",
                geo_occ_rel_tol=args.geo_occ_rel_tol
                if args.model_type in ("geo", "geo_visibility", "geo_visibility_cascade") else "",
                geo_occ_temperature=args.geo_occ_temperature
                if args.model_type in ("geo", "geo_visibility", "geo_visibility_cascade") else "",
                visibility_beta=args.visibility_beta
                if args.model_type in ("geo_visibility", "geo_visibility_cascade") else "",
                visibility_stage2_beta=args.visibility_stage2_beta
                if args.model_type == "geo_visibility_cascade" else "",
...
def main():
    ...
    if (args.visibility_stage2_residual_scale < 0.0 or
            args.visibility_stage3_residual_scale < 0.0):
        raise ValueError("Visibility residual scales must be non-negative.")
    if (args.geo_confidence_mode != "roundtrip" and
            args.model_type not in ("geo", "geo_visibility", "geo_visibility_cascade")):
        raise ValueError(
            "The occlusion confidence mode is implemented only for geo model types.")
    ...
    if args.model_type == "geo_visibility":
        print("Learned visibility: stage1 only, beta={:.3f}".format(
            args.visibility_beta))
    if args.model_type == "geo_visibility_cascade":
        print(
            "Learned visibility cascade: betas=({:.3f}, {:.3f}, {:.3f}), "
            "residual_scales=(0.000, {:.3f}, {:.3f})".format(
                args.visibility_beta, args.visibility_stage2_beta,
