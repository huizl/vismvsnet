#!/usr/bin/env bash
# Compare the supervised visibility head with the best inference-only Geo gate.
# Usage: GPU=2 bash tools/eval_visibility_head_dtu_yao.sh
...
VISIBILITY_CKPT="${VISIBILITY_CKPT:-./checkpoints/dtu/vis_geo_visibility_view5_head/best_2mm.ckpt}"
OUTDIR="${OUTDIR:-./eval/18scan_geo_visibility_head_light3}"
...
MERGED_CSV="${MERGED_CSV:-${OUTDIR}/all_geo_visibility_head.csv}"
...
run_case "geo_roundtrip_stage1_a05" "geo" "${BASE_CKPT}" "0.5"
run_case "geo_visibility_stage1_b05" "geo_visibility" "${VISIBILITY_CKPT}" "0.0"
...
echo
echo "======================================================================"
echo "Completed cases: ${#successful_csvs[@]}/2"
