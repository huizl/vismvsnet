...
from models.vismvsnet_conv_geo import VisMVSModel
from utils import *
...
def _save_error_pfm_and_png(outdir, fname, error):
    """Save error map as PFM and colour-mapped PNG (JET colormap)."""
    pfm_path = os.path.join(outdir, fname.format('.pfm'))
    png_path = os.path.join(outdir, fname.format('.png'))
    os.makedirs(os.path.dirname(pfm_path), exist_ok=True)
...
def save_depth():
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            depth_s3_native = final_depth.squeeze(1).cpu().numpy()
